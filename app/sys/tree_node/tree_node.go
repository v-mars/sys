...
func (r TreeNode) Option() db.Option {
	var o db.Option
...
func (r TreeNode) QueryTreeNode(param QuerySchema) ([]map[string]interface{},error){
	...
	//o.Order = "parent_id,lft ASC"
	o.Scan = true
	dbObj:= db.DBInt(&sys.TreeNode{},o, r.DB)
...
func (r TreeNode) AddNode(obj PostSchema) error{
	...
	var ParentObj sys.TreeNode
	if err:=tx.First(&ParentObj,"id = ? and mark = ?", obj.ParentID,obj.Mark).Error;
	err!=nil{
		return err
	}  // && !errors.Is(err, gorm.ErrRecordNotFound)
	if err:=tx.Model(sys.TreeNode{}).Select("id").Where("`parent_id` = ? and name = ? and mark=?",
	...
	if obj.ParentID == 0 {
		...
		if err:=tx.Model(&sys.TreeNode{}).Select("id").Where("lft = ? and mark=?", 1,obj.Mark).Count(
		...
		var newNode = sys.TreeNode{ParentID: 0, Name: obj.Name, Lft: 1, Rgt: 2,Mark: obj.Mark}
		...
		if err:=tx.Model(&sys.TreeNode{}).Where("rgt >= ? and mark=?",ParentObj.Rgt,obj.Mark).UpdateColumn(
			"rgt", gorm.Expr("rgt + ?", 2)).Error;err!=nil{
			return err
		}
		if err:=tx.Model(&sys.TreeNode{}).Where("lft >= ? and mark=?",ParentObj.Rgt,obj.Mark).UpdateColumn(
			"lft", gorm.Expr("lft + ?", 2)).Error;err!=nil{
			return err
		}
		var right = ParentObj.Rgt+1
		var newNode = sys.TreeNode{
...
func (r TreeNode) MoveNode(obj PutSchema) error {
	...
	if err:=tx.Model(&sys.TreeNode{}).Where(
	...
	*/
	...
	if obj.SiblingID == -1{
		...
		*/
		//if err:=tx.Model(&sys.TreeNode{}).Where("rgt >= ? and id not in (?)",ParentObj.Rgt, moveNodeIdList).UpdateColumn(
		//	"rgt", gorm.Expr("rgt + ?", num)).Error;err!=nil{
		//	response.Error(c, err)
		//	return
		//}
		//
		//if err:=tx.Model(&sys.TreeNode{}).Where("lft >= ? and id not in (?)",ParentObj.Rgt, moveNodeIdList).UpdateColumn(
		...
		//if err:=tx.Model(&sys.TreeNode{}).Where("id in (?)",moveNodeIdList).UpdateColumn(
		...
		/*
			...
			where id in (select id from tree_v2_self_ids)
		*/
		//if err:=tx.Model(&sys.TreeNode{}).Where("rgt > ? and id not in (?)",ParentObj.Lft, moveNodeIdList).UpdateColumn(
		//	"rgt", gorm.Expr("rgt + ?", num)).Error;err!=nil{
		//	response.Error(c, err)
		//	return
		//}
		//
		//if err:=tx.Model(&sys.TreeNode{}).Where("lft > ? and id not in (?)",ParentObj.Lft, moveNodeIdList).UpdateColumn(
		//	"lft", gorm.Expr("lft + ?", num)).Error;err!=nil{
		//	response.Error(c, err)
		//	return
		//}
	...
	if err:=tx.Model(&sys.TreeNode{BaseID: db.BaseID{ID: SelfObj.ID}}).Updates(map[string]interface{}{"parent_id": obj.ParentID}).Error;err!=nil{
		return err
	}
...
func (r TreeNode) DeleteNode(_id string, mark string) error{
	...
	if err:= tx.Delete(&sys.TreeNode{},"lft >= ? and rgt <= ?", NodeObj.Lft, NodeObj.Rgt).Error; err!=nil{
		return err
	}
	var num=NodeObj.Rgt-NodeObj.Lft+1
	if err:=tx.Model(&sys.TreeNode{}).Where("lft > ?",NodeObj.Lft).UpdateColumn(
		"lft", gorm.Expr("lft - ?", num)).Error;err!=nil{
		return err
	}

	if err:=tx.Model(&sys.TreeNode{}).Where("rgt > ?",NodeObj.Rgt).UpdateColumn(
		"rgt", gorm.Expr("rgt - ?", num)).Error;err!=nil{
		return err
	}
