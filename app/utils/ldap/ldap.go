...
func (a *LDAP) Bind(username, password string) error {
	...
	a.Logger.Debugf("ldap user[%s] bind success", username)
	return nil
}
...
func (a *LDAP) Connect() (*ldap.Conn, error) {
	...
	a.Logger.Debugf("ldap addr[%s] conn success", a.Option.Addr)
	return conn, nil
}
...
func (a *LDAP) Authentication(username, password string) (Result, error) {
	...
	if err != nil {
		a.Logger.Errorf("ldap admin bind err: %s", err)
		return r,err
	}
	a.Logger.Debugf("ldap admin bind success")
	entry, err := a.User(username, conn)
	if entry==nil {
		a.Logger.Errorf("Not found ldap user info.")
		return r, fmt.Errorf("Not found ldap user info.")
	}
	a.Logger.Debugf("ldap search user[%s] success", username)
	err = conn.Bind(entry.DN, password)
	if err != nil {
		a.Logger.Errorf("ldap user authentication err: %s", err)
		return Result{},err
	}
	a.Logger.Debugf("ldap user[%s] authentication success", username)
...
func (a *LDAP) User(username string, conn *ldap.Conn) (*ldap.Entry, error) {
	...
	if err != nil {
		a.Logger.Errorf("ldap search err: %s", err)
		return nil, err
	}
	if len(cur.Entries) == 0 {
		a.Logger.Errorf("Not found ldap user.")
...
func (a *LDAP) RestPassword(username, OldPassword, NewPassword string, change bool) error {
	...
	if err!=nil{
		a.Logger.Errorf("ldap user[%s] new password encoder failed.",username)
		return err}
	sql2.Replace("unicodePwd", []string{pwdEncoded})
	//sql2.Replace("userAccountControl", []string{"512"})

	if err = conn.Modify(sql2); err != nil {
		a.Logger.Errorf("ldap user[%s] password modify failed.",username)
		return err
	}
	return nil
}
